...
- AI estimates (SCHEMA_V3 §7): class-only service at `src/services/ai-estimate/` — Facade `AiEstimateService`, Pipeline, Queue, Strategy `EstimateProvider`. Default provider: **Gemini 2.5 Flash** (`GEMINI_API_KEY`, model `gemini-2.5-flash`). Stub via `AI_ESTIMATE_PROVIDER=stub`.
